...
const PostDetailClient = ({ initialPost }: PostDetailClientProps) => {
  ...
  const { data: postData, error: postError, isLoading: swrLoading } = useJobNewsById(params.id as string || null);

  // Use initialPost as fallback until SWR data loads
  const resolvedData = postData || initialPost;
  const loading = initialPost ? (swrLoading && !postData && !initialPost) : swrLoading;

  const post: Post | null = resolvedData || null;
  ...
  // Sync helpful state from data (prefer SWR, fallback to initial)
  useEffect(() => {
    const source = postData || initialPost;
    if (source && !helpfulInitialized) {
      setHelpfulCount(source.helpfulCount || 0);
      setIsHelpful((source as any).userHasMarkedHelpful || false);
      setHelpfulInitialized(true);
    }
  }, [postData, initialPost, helpfulInitialized]);

  // Handle fetch error
  useEffect(() => {
    if (postError && !swrLoading && !post) {
      toast({
        title: 'Error',
