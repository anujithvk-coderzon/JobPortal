...
const JobsPageContent = ({ initialJobs, initialPagination }: JobsClientProps) => {
  ...
  // Fetch jobs on initial load (skip if we have initial data)
  useEffect(() => {
    if (!initialJobs) {
      fetchJobs();
    }
...
const JobsClient = ({ initialJobs, initialPagination }: JobsClientProps) => {
  return (
    <Suspense fallback={
      ...
      <JobsPageContent initialJobs={initialJobs} initialPagination={initialPagination} />
    </Suspense>
  );
};
